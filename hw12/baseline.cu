...
void poolReluConv1(float* input, int inputRowSize, int inputColSize, \
    ...
    float* output, int outputRowSize, int outputColSize, float* d_outputTmp, cudaStream_t stream)
{
    ...
    conv2d(input, inputRowSize, inputColSize, inputChannel, \
        ...
        d_outputTmp, inputRowSize - kernelConv1Size + 1, inputColSize - kernelConv1Size + 1, outputChannel, stream);
    ...
    printf("--------------------------------output_conv--------------------------------\n");
    printTensor(outputTmp, inputRowSize, inputRowSize, inputChannel);
    reluMaxPool(d_outputTmp, inputRowSize, inputColSize, inputChannel, \
...
int main(int argc, char* argv[]) {
    ...
    // printTensor(input, 28, 28, 1);
        ...
        poolReluConv1(d_input + d_input_size * stream_tid, 28, 28, \
            ...
            streams[stream_tid]);
        cudaMemcpy(output1.data(), d_output1, output1.size() * sizeof(float), cudaMemcpyDeviceToHost);
